...
const Stop = ({ stop, index }) => {
    ...
    return (
        <article className="stop">
                        ...
                        <ul className="stop__departures-list">
                            {departures.map((departure, index) => (
                                <li key={index} className="departure">
                                    ...
                                    {departure.delay > 0 && (
                                        <small className="time-info time-info--late">
                                            +{departure.delay}s
                                        </small>
                                    )}
                                    {departure.delay < 0 && (
                                        <small className="time-info time-info--soon">
                                            {departure.delay}s
                                        </small>
                                    )}
                                    <p className="departure__direction">
                                        <span>→</span> {departure.direction}
                                    </p>
                                </li>
                            ))}
                        </ul>
            ...
            {noDepartures && (
                <p className="no-departures-found">{noDepartures}</p>
            )}
        </article>
    );
};
